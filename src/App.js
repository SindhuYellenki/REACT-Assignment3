...
import EndCard from './Component/EndCard'
...
class App extends Component {
  ...
  changeActiveTab = id => {
    console.log('changeTabTriggerred')
    this.setState({activeTab: id})
  }

  getThumbNailId = id => {
    const {randomImage, gameActive} = this.state
    if (randomImage.id === id) {
      this.setState(p => ({
        score: p.score + 1,
      }))
      const NewRandomImage = Math.floor(Math.random() * 30) + 1
      this.setState({randomImage: imagesList[NewRandomImage]})
    } else {
      this.setState({gameActive: false})
      clearInterval(this.timerId)
    }
  ...
  render() {
    const {score, time, activeTab, randomImage, gameActive} = this.state
    console.log(time, gameActive)
                ...
                <ul className="Tab-Images-Container">
                  {filteredList.map(each => (
                    <TabImages
                      details={each}
                      key={each.id}
                      getThumbNailId={this.getThumbNailId}
                    />
                  ))}
                </ul>
              </nav>
